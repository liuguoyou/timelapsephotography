 ...
 */
void shrinkImage(const IplImage * srcImage, IplImage ** dstImage){

	cvPyrDown(srcImage, *dstImage, CV_GAUSSIAN_5x5);
}
	...
	if(shiftBits > 0){
		...
		for(i = -1; i < 1; i++)
			for(j = -1; j < 1; j++) {
				...
				//cvSaveImage("images/results/diff_o.jpg", diffBMP, 0);
				
				andBitMap(diffBMP, eb1, diffBMP);
				//cvSaveImage("images/results/diff_1.jpg", diffBMP, 0);
					
				andBitMap(diffBMP, shiftedEB2, diffBMP);
				//cvSaveImage("images/results/diff_2.jpg", diffBMP, 0);
